...
public class SkillConfig
{
	...
	public string Identifier
	{
		...
		set
		{
			int num = Math.Abs(StringExtensionMethods.GetStableHashCode(value));
			if (num > 999)
			{
				_identifier = value;
				UID = (Skills.SkillType)num;
				return;
			}
	...
	/// </summary>
	public float IncreaseStep { get; set; } = 1f;
	...
	{
		if (!string.Equals(Name, name, StringComparison.CurrentCultureIgnoreCase) && !(Identifier == name))
		{
			return string.Equals(Localization.instance.Localize(Name), name, StringComparison.CurrentCultureIgnoreCase);
		}
		return true;
	}
	...
	{
		return global::SimpleJson.SimpleJson.DeserializeObject<SkillConfig>(json);
	}
	...
	{
		return global::SimpleJson.SimpleJson.DeserializeObject<List<SkillConfig>>(json);
	}
}
